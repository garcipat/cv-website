...
- **FR-038**: System MUST display a sign's hint text in a speech-bubble tooltip near the character when the character presses Up (or `W`) — the same "interact" convention chests already use — while overlapping the sign's trigger zone, and hide it (via a grow/fade-out animation) once the character walks away from the sign. *Amended 2026-08-31 — live UX feedback during implementation replaced automatic reveal-on-overlap with an explicit Up/`W`-press trigger: since this step's whole point is teaching an obscure control (e.g. Down/`S` to drop through a one-way bridge), the reveal itself can't assume the player already knows the interact convention; see `specs/S-006-platformer-theme/plans/2026-08-29-hint-signs.md`'s "Revised 2026-08-31" preamble for the full rationale.* This interaction MUST NOT pause the game, MUST NOT block movement (signs are not solid), and MUST NOT add anything to `collectedFacts` or the journal — hint text is not CV data, and signs are reusable rather than consumed on first touch.
