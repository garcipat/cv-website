...
### User Story 4 - Defeat Enemies to Reveal Courses, or Collect a Key from a Purple Slime (Priority: P2)

Scattered through the level are two colors of simple slime enemies that patrol back and forth on the ground. Green slimes guard the Courses pool from the CV — every Course fact is delivered by a green slime. Purple slimes are larger and slower than green slimes, and carry no CV content: defeating one pops a key out onto the ground beside it instead of revealing a CV fact. The key sits as its own bobbing pickup (identical bobbing motion to a coin) until the character walks into it, at which point it's added to the visitor's key count and can later be spent to open a chest (see User Story 6). The visitor defeats either slime color by jumping on top of it (stomp mechanic) — a green slime falls in a single stomp; a purple slime, being tougher, takes three. When defeated, the enemy disappears in a poof animation; a green slime's Course fact text floats up from the defeat point, hovers briefly, then animates toward the journal icon; a purple slime instead pops its key onto the ground, which the character then collects by touch like any other ground pickup, at which point it animates toward the key counter in the HUD. Defeated enemies stay defeated for the session; an uncollected dropped key also stays in the world for the rest of the session (it does not disappear or expire if left behind).

**Why this priority**: Enemies add gameplay depth and reward exploration. They're P2 because coins already deliver Skills — enemies unlock Courses (green) and the keys chests require (purple), both of which need skilled play to discover.

**Independent Test**: Approach an enemy — verify it patrols. Jump on top of a green slime — verify it's defeated in one stomp with a poof effect and a Course fact appears. Jump on a purple slime three times — verify it's visibly larger and patrols more slowly than a green slime, takes three stomps before defeat, and drops a key that bobs in place on the ground. Walk away without touching the key — verify it's still there when you come back. Walk into the key — verify it flies to the HUD key counter, which increments, and nothing is added to the journal. Open journal — verify Course facts appear in the Courses section. Revisit the area — verify enemies are gone.

**Acceptance Scenarios**:

1. **Given** a green slime is patrolling on the ground, **When** the character jumps and lands on top of it, **Then** it is defeated in a single stomp with a poof/squish animation and the associated Course fact text floats up, hovers, and flies to the journal icon.
2. **Given** a purple slime is patrolling on the ground, **When** the character stomps it three times (a bounce off each of the first two stomps, then a third, finishing landing), **Then** it is defeated on the third stomp with a poof/squish animation and a key pickup appears on the ground at the defeat point, bobbing up and down in place like a coin — no CV fact is revealed and nothing is added to the journal yet.
2b. **Given** a dropped key is sitting on the ground, **When** the character's hitbox overlaps it, **Then** the key disappears from the world, its icon floats up and flies to the HUD key counter, and the key counter increments by one.
2c. **Given** a dropped key is sitting on the ground, **When** the character walks away without touching it, **Then** the key remains in place, still bobbing, for the rest of the session (or until collected).
3. **Given** an enemy is defeated, **When** the visitor opens the journal, **Then** green-slime defeats appear in the Courses section as simple list entries; purple-slime defeats and key pickups never appear in the journal.
...
Scattered through the level (via hand-placed markers, same pattern as coins/enemies/blocks — no special end-of-level positioning) are treasure chests: the level's "main objective" collectible, visually and mechanically distinct from coins/fruit/crates. Each chest is associated with an Experience entry from the CV — the section considered most valuable, and therefore worth the extra weight of a dedicated collectible type. A chest starts closed, sits flush with the ground (not a solid obstacle — the character walks over/beside it, not on top of it). Unlike every other collectible, a chest does NOT open on touch: the character must be standing on/overlapping it, hold at least one collected key, and press Arrow Up to open it — a deliberate small pause-and-choose moment for the level's main objective, distinct from the passive walk-through-it collection every other pickup uses. Keys are earned by defeating purple slimes (User Story 4) and shown as a running count in the HUD. Opening a chest spends one key (decrementing the HUD key counter by one), permanently swaps the chest to its open state, and reveals the associated Experience fact, which floats up and flies to the journal icon exactly like any other collectible. Pressing Arrow Up on a closed chest while holding zero keys has no effect — the chest stays closed. A dedicated chest counter in the HUD (e.g. "Chests 2/5") tracks progress separately from the heart/coin/key HUD elements.
...
**Independent Test**: With zero keys collected, stand on a closed chest and press Arrow Up — verify nothing happens and the chest stays closed. Defeat a purple slime to collect a key — verify the HUD key counter increments. Stand on the closed chest and press Arrow Up again — verify it swaps to its open sprite, the key counter decrements by one, the chest counter increments, and the Experience fact flies to the journal. Walk over (don't press Up on) a closed chest — verify it stays closed. Open every chest in the level — verify the Thank You screen appears, pausing the game, showing the thank-you message and Contact info with "press any button to continue". Press any key — verify the screen closes and gameplay resumes from the same position. Open the journal — verify Contact never appears as a bookmark.

**Acceptance Scenarios**:

1. **Given** a closed chest exists in the level and the visitor holds at least one key, **When** the character is standing on/overlapping the chest and the visitor presses Arrow Up, **Then** it permanently swaps to its open sprite (never re-closes for the rest of the session), the HUD key counter decrements by one, the HUD chest counter increments, and the associated Experience fact text floats up and flies to the journal icon.
1a. **Given** a closed chest exists in the level and the visitor holds zero keys, **When** the character is standing on/overlapping the chest and the visitor presses Arrow Up, **Then** nothing happens — the chest stays closed, no key is spent, and no fact is revealed.
...
2. **Given** some but not all chests are open, **When** the visitor checks the HUD, **Then** the chest counter shows the count of opened vs. total chests in the level (e.g. "Chests 2/5"), and the key counter shows how many keys are currently held.
...
Some parts of the level are only reachable by climbing a ladder — a `'ladder'` terrain tile, visually distinct from ordinary solid terrain, that the character can pass through horizontally (not solid) but ascend/descend while overlapping it. Standing on a ladder tile and pressing Arrow Up (or `W`) starts climbing upward; pressing Arrow Down (or `S`) starts climbing downward. While climbing, gravity is suspended and vertical movement is driven directly by Up/Down at a fixed climb speed; horizontal movement (Left/Right) still works exactly as normal (free-form climbing — the character can shimmy sideways off a ladder mid-climb, which ends the climb and returns to normal gravity the instant no part of the character's hitbox still overlaps a ladder tile). Pressing Space (jump) while climbing immediately ends the climb and applies a normal jump impulse. The ladder's own topmost tile is solid-from-above, one-way exactly like a `bridge` tile (FR-006) — not a separate solid tile placed above the shaft: reaching the top rung with nothing climbable or solid directly above it lands the character standing on that same rung; pressing Down while standing there re-enters the climb downward (mirroring the bridge drop-through convention, but for `'ladder'` tiles instead of `'bridge'`).
...
4. **Given** the character climbs to the ladder's top rung, **When** they reach its top edge with nothing climbable or solid directly above, **Then** they stand on the ladder's own top tile, which becomes solid-from-above via a dedicated special case (`isStandableLadderTop`), not FR-006's plain generic ground-collision rule.
5. **Given** the character is standing on the ladder's own top rung (no separate tile above it), **When** the visitor presses Arrow Down (or `S`), **Then** the character re-enters the climb, moving downward into the ladder.
...
2. **Given** the controls overlay is visible, **When** the visitor walks about two tiles away (in either direction) from where they stood when it appeared, **Then** the overlay disappears and does not reappear for the rest of the session. There is no keypress trigger or timeout fallback.
...
- **FR-006**: System MUST implement character physics:
  ...
  - **Climbing**: `'ladder'` terrain tiles are not solid (they don't block horizontal movement or count as ground) but are climbable — while the character's hitbox overlaps one, pressing Up or Down suspends gravity and drives vertical movement directly at a fixed climb speed instead. Horizontal movement stays exactly as normal while climbing (free-form — moving off every overlapping `'ladder'` tile ends the climb immediately, resuming normal gravity/collision). Pressing Space while climbing ends it and applies a normal jump impulse. A ladder shaft's topmost tile, when nothing climbable or solid sits directly above it, becomes standable from above itself — a dedicated special case (`isStandableLadderTop`) mirroring `bridge`'s one-way contract, rather than requiring a separate solid tile placed above the shaft; pressing Down while standing there re-enters the climb downward, mirroring the bridge drop-through convention above but keyed off `'ladder'` instead of `'bridge'`.
...
- **FR-008**: System MUST construct a single continuous level (for v1) from left to right. The level consists of:
  - **Terrain tiles**: Ground, walls, bridges, ladders — solid or climbable blocks the character interacts with (see FR-006). Ground tiles render as grass on any top-exposed face and as buried rock/dirt otherwise; there is no separate "platform" tile kind — a floating strip of ground uses the same ground tile as the main terrain.
  - **Collectibles (coins)**: Placed on the ground and in the air at varying heights.
  - **Enemies** (P2): Patrol enemies on the ground.
...
- **FR-009**: System MUST map CV sections to specific game object types — each collectible type reveals content from assigned sections only:
  ...
  - **Green slimes** → Courses (User Story 4).
  - **Purple slimes** → no CV mapping; defeating one drops a key, spent to open a chest (User Story 4, User Story 6).
...
- **FR-019**: System MUST render simple enemy characters that patrol horizontally on the ground. Enemies reverse direction at ledge edges or designated patrol boundaries. Purple slimes render at 1.5× a green slime's size and patrol at roughly 70% of a green slime's speed; each slime's hitbox matches its own rendered size (a bigger purple slime has a proportionally bigger hitbox).

- **FR-020**: System MUST implement enemy interaction:
  - **Stomp defeat**: Character landing on top of an enemy defeats it with a poof/squish animation — a green slime is defeated in a single stomp, reveals its associated Course fact, and the fact floats to the journal icon; a purple slime has 3 hit points and takes three stomps to defeat, revealing nothing until the third, finishing stomp, at which point it pops a key onto the ground at the defeat point instead of a CV fact — the key is a separate ground pickup, collected by touch, not flown to the HUD automatically (see FR-020e). Each stomp (finishing or not) gives the character a short upward bounce.
...
- **FR-023**: System MUST render treasure chests as a `ChestDef` entity, placed via hand-authored level markers (the `T` marker, consistent with FR-013's marker-based placement approach) — one chest per non-empty Experience entry. A chest sits flush with the ground (not solid — FR-006's platform-collision rules don't apply to it) and renders in its closed state by default. Unlike every other collectible, opening a chest is NOT automatic on touch and requires a collected key — when the character's hitbox overlaps a closed chest, the visitor holds at least one key, AND the visitor presses Arrow Up, the chest spends one key (the HUD key counter decrements by one), permanently swaps to its open state (never reverts to closed except via Reset Game), and the associated Experience fact text floats up from the chest, hovers briefly, then animates toward the journal icon, same as any other collectible. Pressing Arrow Up while holding zero keys, or merely overlapping a closed chest without pressing Arrow Up, has no effect. A HUD chest counter (e.g. "Chests 2/5", positioned near the heart/coin HUD elements) tracks how many chests have been opened out of the level's total; a separate HUD key counter tracks keys currently held (see FR-020e). Per FR-013's marker-count convention, the level's actual chest count is capped by how many `T` markers exist, not by how many Experience entries `CVData` has. Chest defs zip against markers in level-reading order against Experience entries in **reversed** `CVData` order (oldest entry first) — since `CVData.experience` is stored newest-first but markers are placed near-spawn-to-farther-away, this makes the chests read as a chronological career progression as the visitor plays further into the level: the closest chest reveals the oldest job, the farthest chest the newest one.

- **FR-020e**: System MUST track a `collectedKeys` count in game session state, starting at 0. Collecting a dropped key (FR-020, touching a defeated purple slime's key pickup) increments it by one; opening a chest (FR-023) decrements it by one. A HUD key counter, positioned near the other HUD elements, is hidden while `collectedKeys` is 0 and appears showing the current count as soon as it rises above 0 — unlike the always-visible heart/journal HUD elements, the key counter only takes up HUD space once the visitor actually has a key to track. `collectedKeys` and any not-yet-collected dropped key pickups persist across a death/respawn (FR-020b) — matching `chestStates`/`blockStates`/`collectedFacts`'s existing persist-through-death convention, not `enemyStates`'s reset-on-death one (a purple slime itself does respawn alive again on death, same as every other enemy, independent of whether its key was already dropped and collected in a prior life). Both are cleared only by a deliberate Reset Game (FR-018b), consistent with how `collectedFacts`/`chestStates`/`bonusFruitStates` already reset there. A given purple slime drops at most one key ever, deduplicated by reusing the enemy's own id as its key pickup's id (mirroring `spawnBonusFruit`'s id-reuse convention) — since `enemyStates` itself DOES reset on death (a purple slime respawns alive again, same as every other enemy), defeating the same respawned slime again in a later life must not drop a second key. This is the same anti-duplication shape FR-020c already establishes for facts/fruit: a respawned enemy's repeat defeat yields nothing extra if its reward was already granted. Keys are rendered from a dedicated `key.png` sprite (pixel-art icon, transparent background), reused for both the world pickup drop and the HUD counter icon; the world pickup bobs up and down in place exactly like a coin (reusing `Coin.ts`'s `coinBobOffset`) until collected.
...
- **FR-036**: System MUST show a translucent controls overlay listing only universal controls (movement keys, jump key, journal toggle key) when gameplay first enters the `playing` phase. Dismissal is purely distance-based: the overlay MUST auto-dismiss once the player has traveled a short distance (about two tiles, in either direction) from where they stood when it appeared, and MUST NOT reappear for the remainder of the session. There is no keypress trigger and no timeout fallback — a visitor who never moves keeps seeing the overlay indefinitely, by design.
...
- **FR-038**: System MUST display a sign's hint text in a speech-bubble tooltip near the character when the character presses Up (or `W`) — the same "interact" convention chests already use — while overlapping the sign's trigger zone, and hide it (via a grow/fade-out animation) once the character walks away from the sign. The reveal requires this explicit press rather than firing automatically on overlap because a hint sign's whole point is teaching an obscure control (e.g. Down/`S` to drop through a one-way bridge), so the reveal itself can't assume the player already knows the interact convention it's demonstrating. This interaction MUST NOT pause the game, MUST NOT block movement (signs are not solid), and MUST NOT add anything to `collectedFacts` or the journal — hint text is not CV data, and signs are reusable rather than consumed on first touch.
...
- **FR-031**: System MUST manage game session state containing:
  ...
  - `enemyStates`: Per-enemy position, direction, defeated status, remaining hit points (P2)
  - `blockStates`: Per-block hit count and broken status (P2)
  - `chestStates`: Per-chest open/closed status (P2)
  - `keyPickupStates`: Per-dropped-key position and collected status — one entry created per purple-slime defeat (P2)
  - `collectedKeys`: Count of keys currently held, spent one-at-a-time to open chests (P2, see FR-020e)
...
- **FR-032**: System MUST define TypeScript types for the platformer theme:
  ...
  - `TileType`: `'groundGrass' | 'groundRock' | 'wall' | 'bridge' | 'ladder' | 'empty'` — `ladder` is not solid (`isSolid('ladder') === false`) but climbable (`isClimbable(tile)`). Ground tiles alone represent walkable terrain, whether part of the main ground or a floating strip — there is no separate "platform" tile kind; `groundGrass` renders its top-exposed vs. buried sprite automatically based on what sits above it.
  ...
  - `EnemyDef`: `{ id: string; spriteType: 'slimeGreen' | 'slimePurple'; fact?: CollectedFact }` — `fact` is present on `slimeGreen` (drawn from Courses); `slimePurple` carries no `fact` and instead drops a key pickup on its finishing stomp (see `KeyPickupDef`).
  ...
  - `ChestDef`: `{ id: string; fact: CollectedFact }`, mirroring `EnemyDef`'s static-def shape — `fact` is always present (drawn from Experience, one chest per non-empty entry). Placement adds `x`/`y` to produce `ChestPlacement extends ChestDef`, same `Def`→`Placement` convention as `EnemyPlacement`/`CollectiblePlacement`/`BlockPlacement`. Live per-instance open/closed state (`ChestState extends ChestPlacement { state: 'closed' | 'open' }`) mirrors `BlockState`'s `Def`→`Placement`→`State` layering. Opening a chest additionally requires `collectedKeys > 0` and decrements it by one (FR-023).
  - `KeyPickupDef` / `KeyPickupState`: `{ id: string; x: number; y: number; collected: boolean }` — created dynamically (not from a level marker) the moment a purple slime's third stomp lands, at the defeat point; bobs like a coin (`coinBobOffset`) until the character's hitbox overlaps it, at which point `collected` flips to `true`, it's removed from the world, and `collectedKeys` increments by one.
  ...
  - `SignDef`: `{ id: string; hintId: string }` — `hintId` looks up localized text at `platformer.hints.<hintId>` in the existing i18n translation files; no `cvSection`/`cvIndex`, since signs carry no CV mapping (P3, see FR-037). Placement adds `x`/`y` to produce `SignPlacement extends SignDef` (`level/SignMapper.ts`), same `Def`→`Placement` convention as `EnemyPlacement`/`CollectiblePlacement`/`BlockPlacement`.
...
- **FR-033**: System MUST include unit tests covering:
  ...
  - `EnemyMapper` — correct mapping of Courses to green slimes sequentially by index; purple slimes carry no fact and are never assigned a Course
  - `KeyPickup` mechanics — a purple slime's finishing stomp spawns a `KeyPickupState`; touching it increments `collectedKeys` and removes the pickup; bobbing reuses `coinBobOffset`
...
- **Collectible**: An item in the game world that, when acquired by the player, reveals a CV fact. Types: coin (Skills), green-slime defeat (Courses), crate block break (Education/Activities/Languages), question-mark block bonus fruit (Certificates/Projects). Each collectible is mapped to a specific CV data item by section. A dropped key (from a purple-slime defeat) is collected the same way as any ground pickup but carries no CV fact — it's spent on chests, not shown in the journal.
...
CVData (from currentCV signal)
 ...
 ├── Mapped by EnemyMapper → EnemyDef[] (courses on green slimes; purple slimes carry no fact, placed in level)
...
Level
 ...
 ├── Contains Terrain (ground/wall/bridge/ladder tiles, collision data)
 └── Defines spawn and checkpoint positions

Player
 ├── Collides with Terrain (stands, jumps, falls, climbs)
 ├── Collides with Collectible/Block/KeyPickup → triggers fact/key collection
 ├── Collides with Chest → opens it (on Arrow Up, if collectedKeys > 0), triggers fact collection
...
Session State
 ...
 ├── keyPickupStates / collectedKeys — dropped keys in the world and keys currently held
...
- **Sprite assets from existing mockup images**: Character, enemy, coin, terrain, and block sprites derive from the images in `specs/S-006-platformer-theme/images/`. Chest sprites (`chest_closed.png`/`chest_open.png`) and the key sprite (`key.png`) were generated separately and checked in under `public/sprites/` (cropped to their tight non-square bounding box and downscaled with nearest-neighbor sampling, transparent background — chests 28×20 / 24×20, key 14×28).
...
- **Q: Enemy section mapping** — **A**: Courses (12 CV entries) sit on enemies rather than sharing the crate mechanic with Education/Activities — too large a pool for a stomp mechanic, and slimes are well suited to a large pool. Green slimes alone deliver the Courses pool, sequentially by index. Purple slimes carry no CV content at all — they're a tougher, key-dropping variant that gates chest-opening instead (User Story 6). See FR-009 and User Story 4.
