...
- ✅ **The tile under a marker is erased**: right-click, or the Eraser tool, clears the tile and the cell's marker together, whatever the marker's kind. Painting a *specific* tile over the cell leaves a patrol boundary or connection point in place, but removes a marker that describes the replaced tile (a sign, falling stalactite or torch).
...
- **FR-002**: A cell MUST be able to hold a terrain tile and a marker at the same time, and a marker MUST also be able to share a cell with an entity, sign or hazard marker. Painting a marker MUST NOT change, remove or replace anything in that cell. Painting a specific foreground tile over a cell (a left-click) MUST NOT remove a terrain-independent marker (a patrol boundary or a connection point), but MUST remove a marker that describes the tile it replaced (a sign, a falling stalactite or a torch). Erasing a cell — right-click, or the Eraser tool — MUST remove that cell's marker whatever its kind, so metadata can always be cleared.
