...
- [x] T004 [P] Create the registry types in `src/themes/platformer/engine/effects/registry.ts`: `EffectKind` = `flight | counterPopup | puff | healAura | hitSplatter | fadeOutText | explosion | debris`; `EffectLayer` = `midWorld | worldEffects | aboveWorld | hudLast`; `EffectResetScope` = `death | progress`; `EffectRegistryEntry<S>` with `kind`, `create`, optional `tick`, `draw`, optional `expired`, `layer`, `resetScope`, optional `keyOf`. Document that declaration order is significant and fixes intra-layer draw order.
...
- [x] T008 [P] [US1] Create `src/themes/platformer/engine/effects/flight.ts`: move `FlightEffect`, `RISE_DURATION_SECONDS`, `HOLD_DURATION_SECONDS`, `FLIGHT_DURATION_SECONDS`, `startFlightEffect`, `tickFlightEffect` (4-phase rising→holding→flying→done, `phase === 'done'` terminal), `flightEffectPosition`, plus `COLLECTION_TEXT_SLOT_COUNT`, `COLLECTION_TEXT_STACK_ROW_HEIGHT`, `SlotAllocator`, `createSlotAllocator`. Add the family's `draw` (the body of `Renderer.ts`'s `drawCollectionEffects`).
...
- [x] T016 [US1] Register all eight kinds in declaration order in `src/themes/platformer/engine/effects/registry.ts`, wiring each module's `create`/`tick`/`draw`/`expired` with its `layer` and `resetScope` from the `data-model.md §5` table, in an order whose intra-layer sequence matches `flight` → `puff` → `debris` → `hitSplatter` → `fadeOutText`: `flight` (worldEffects/progress, override expired `phase === 'done'`), `counterPopup` (hudLast/progress, keyed by `labelKey`, override tick returns `null`), `puff` (worldEffects/progress), `healAura` (midWorld/progress), `debris` (worldEffects/progress), `hitSplatter` (worldEffects/progress), `fadeOutText` (worldEffects/**death**), `explosion` (aboveWorld/progress).
...
- [x] T019 [US1] Replace the eight per-kind effect signals in `src/themes/platformer/PlatformerState.ts` (`activeFadeOutTexts`, `activeExplosions`, the flight-only `activeEffects`, `activePuffs`, `activeHealAuraEffects`, `activeHitSplatters`, `activeCounterPopups`, `activeDebrisEffects`) with one `export const activeEffects = signal<TransientEffect<unknown>[]>([])`; add the registry-driven `spawnEffect` helper (append; keyed kinds replace-in-place — see US5) and retarget all imports to `engine/effects` (FR-003). The four timed-tile timer signals remain separate.
- [x] T020 [US1] Update `src/themes/platformer/state/rewards.ts` to spawn flight effects and counter popups through the new modules/`spawnEffect` and read `effectCount(activeEffects.value, 'flight')` for the slot allocator seed (replacing the old flight-only signal length) (FR-003).
- [x] T021 [US1] Update `src/themes/platformer/PlatformerPage.tsx` tick: delete the six byte-identical per-kind tick bodies and the flight/counter transition plumbing; call exactly one `advanceEffects(activeEffects.value, dt)` per `playing` tick (FR-004/FR-007), and retarget every effect spawn site (flight text, puffs, heal aura, hit splatters, fade-out labels, explosions, debris, counter popups) to push into the unified collection.
...
- [x] T029 [P] [US1] Write `src/themes/platformer/engine/effects/registry.test.ts`: the registry contract — a throwaway kind registered with one module + one registry line starts, advances, draws at its declared layer, and expires without any edit to state, tick, draw, or reset code (US1-5/SC-003).
- [x] T030 [P] [US1] Write boundary tests in `src/themes/platformer/engine/effects/flight.test.ts` and `src/themes/platformer/engine/effects/counterPopup.test.ts`: the 4-phase machine transitions at exactly RISE/HOLD/FLIGHT, is a no-op once `done`, and the counter popup returns the `null` sentinel at `elapsed >= duration`.
...
```bash
Task: "Create engine/effects/flight.ts per T008"
