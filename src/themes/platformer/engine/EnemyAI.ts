 ...
 * the enemy's own grid row, is either a wall or has no solid ground beneath
 * it (a ledge/pit edge) — FR-019's "reverse at platform edges or designated
 * patrol boundaries". A tile counts as solid for both checks if EITHER the
 * static terrain grid says so (`isSolid`/`tileAt`) OR it matches an entry in
 * `blockedTiles` — the currently-live `crate`/`questionMark`/`fragileRock`
 * blocks, which `LevelParser.ts` deliberately resolves to `'empty'` terrain
 * since they're a separate dynamic layer (`BlockState`) the static grid
 * knows nothing about; without this, an enemy patrolling onto or beside a
 * live block would see a false wall/ledge from the terrain grid alone.
 * Enemies never move vertically; `row` is derived once from `enemy.y` and
 * never changes (a flat patrol row, no gravity — this is a deliberately
 * simple patrol-only AI per spec.md's "Boss enemies or complex enemy AI"
 * non-goal).
 */
