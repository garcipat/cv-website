...
export const PHYSICS_CONFIG = {
  ...
  /**
   * Upward velocity impulse applied to the player on destroying a pot by
   * landing on it, in px/s (negative = up) — weaker than the enemy-stomp
   ...
   * (-150) so there's enough hang-time to actually see the dropped pickup
   * land before the player comes back down. Shared by every pot kind (see
   * entities/blocks/pot.ts's `createPotType`). Gated by
   ...
   * `Math.abs(potBounceVelocity) * MAX_DT` must stay below
