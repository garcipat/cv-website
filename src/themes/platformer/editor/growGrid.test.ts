...
describe('growGrid', () => {
  ...
  it('worksGenericallyForAnotherCharUnion-suchAsBackgroundChar', () => {
    const bgGrid: BackgroundChar[][] = [['d', 'd']];
    const result = growGrid(bgGrid, 2, 0, '.');
    expect(result.grid).toEqual([['d', 'd', '.']]);
  });
});
