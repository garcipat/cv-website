 ...
 * `education`/`certificates` because their entries are long and may need the
 * full page spread each; `skills` too, because a category's skill list (with
 * star ratings, see `formatJournalEntry`) reads better one category per page
 * than all categories crammed into one scrolling list. `languages`/`courses`
 * are grouped instead — a language entry is a single short "Name ★★★★☆"
 * line and a course entry is one short title+subtitle line (same shape, see
 * `formatJournalEntry`), so all of them fit together on one page without
 * needing page controls; one full page per course felt like far too much
 * for how little each entry says.
 */
