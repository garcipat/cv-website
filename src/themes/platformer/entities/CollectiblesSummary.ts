 ...
 * own counters already use, so both places always agree (amended 2026-08-30,
 * live user feedback: this summary used to total against raw CVData length,
 * which could show more than the level actually has placed — e.g. Projects
 * showing "0/3" forever when 0 of its 3 entries have a question-mark marker
 * yet).
 */
 ...
 * another bookmark page.
 *
 * Amended 2026-08-30 (live user feedback during step 21 verification, after
 * the Certificates/Projects/Courses remap): "coins" is still Skills;
 * "fruits" now counts Certificates+Projects (question-mark blocks' bonus
 * fruit, moved off enemies — see `BlockMapper.ts`'s
 * `certificateToBlock`/`projectToBlock`), not Languages (unmapped for now);
 * "enemies" now counts Courses (both slime colors guard the same pool — see
 * `EnemyMapper.ts`'s `courseToEnemy`), not Certificates+Projects. Keyed by
 * `sectionId` rather than `sourceType` for "fruits"/"coins" (a fact's
 * `sourceType` alone can't distinguish "which pool", e.g. `'block'` also
 * covers crates) — "enemies" can still key on either since Courses is the
 * only enemy-sourced section today, but `sectionId` is used for consistency
